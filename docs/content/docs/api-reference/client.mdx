...
| Module | Purpose |
| --- | --- |
| `checkout` | Headless orchestration of the full MVP sales flow (cart, invoice, shipping assignment, payment initialization). |
...
```ts
import { products, categories, cart, checkout } from '@sazito/client-sdk';
...
const checkoutApi = checkout({ domain: 'mystore.sazito.com' });
```

All available aliases:
`products`, `categories`, `cart`, `checkout`, `orders`, `invoices`, `shipping`,
