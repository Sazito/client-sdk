...
Cart line variant attributes are returned as `cart.items[].product.attributes`.
Each value is either a string or a `ProductAttributeValueObject`; when
`fieldType` is `'color'`, use the hex payload in `value.extra` to render a
color swatch. These are display attributes and are separate from
`formAttributes` and `schedulerBookingAttributes`.
