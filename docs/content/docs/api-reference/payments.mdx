...
For `pending`, call `pollUntilSettled()` (15 seconds by default). Polling stops when
the action changes to `show_order` or a failure action.
...
Its invoice requires `netTotal` and `finalTotal`. Items use `variantAttributes`,
