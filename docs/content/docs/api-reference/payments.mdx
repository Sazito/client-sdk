...
| `order` | `Order` | Optional | Required by both `show_order` and `pending`. Response keys inside the order are camel-cased once by the SDK. |
...
Type: `PaymentActionBase & (   \| { action: 'POST'; address: string; payload: Record<string, string \| number> }   \| { action: 'REDIRECT'; address: string }   \| { action: 'UPLOAD'; time?: number }   \| { action: 'show_otp_modal'; time?: number }   \| { action: 'show_order'; order: Order }   \| { action: 'pending'; order: Order }   \| { action: 'payment_fail_error' \| 'show_error' \| 'FAIL' }   \| { action: 'StockViolated' } )`
