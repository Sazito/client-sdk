...
Verifies a returning gateway payment with Sazito's SSR-compatible URL-encoded
form contract. Gateway values are sent as `payload[fieldName]`. When both
callback credential fields are provided, the SDK restores them before making
