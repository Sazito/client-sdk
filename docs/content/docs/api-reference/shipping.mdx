...
| `address.mobilePhone` | `string` | Optional | Recipient mobile phone. |
...
export default function Example() {
  ...
  useEffect(() => {
    sazito.shipping.getMethods().then((res) => setData(res.data ?? null));
  }, []);
...
onMounted(async () => {
  const res = await sazito.shipping.getMethods();
