...
- Use `createAddress` for checkout. Each submitted address must be a new,
...
- Do not use `updateAddress()` in checkout: the backend mutates the referenced
  row, which can change the address shown on previous orders.

Shipping rates expose their own `color` field, while product variant
attributes shown beside shipping items come from the invoice line item. Those
items use the shared `ProductAttribute` shape, including `value.extra` for a
color hex payload and `fieldType: 'color'` for color fields.
...
> Deprecated for checkout. This mutates the existing address row. Use
> `createAddress()` to preserve historical order snapshots.
...
Returns the authenticated user's normalized addresses newest first. This
request is not cached, so the first item is safe to use as checkout prefill.
