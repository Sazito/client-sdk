...
- `get` uses the `refresh` contract under the hood.
...
Invoice line attributes are returned as `invoice.items[].attributes` using the
shared `ProductAttribute` shape. A color value may be a
`ProductAttributeValueObject` with `fieldType: 'color'` and a hex payload in
`value.extra`; use its nested `value` for the human-readable label.
...
Attaches checkout dynamic form data to invoice via the invoice form endpoint.
