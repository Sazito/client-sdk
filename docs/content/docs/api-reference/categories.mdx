...
export default function Example() {
  ...
  useEffect(() => {
    sazito.categories.list({ page: 1, pageSize: 20 }).then((res) => setData(res.data ?? null));
  }, []);
...
export async function runExample() {
  ...
  const res = await client.categories.list({ page: 1, pageSize: 20 });
...
onMounted(async () => {
  const res = await sazito.categories.list({ page: 1, pageSize: 20 });
