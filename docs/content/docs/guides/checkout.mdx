...
the GET or POST fields, verifies the payment, and sends a `303` redirect to the
configured `checkoutPath`. The checkout page then reads payment status and
shows the result. If the status is pending, polling starts automatically.
