...
`parsePaymentReturn` validates the callback shape against Sazito's known gateway
result markers, maps the payment ID and
...
The verification request is URL encoded according to Sazito's server contract;
pending status checks remain JSON requests. To support a custom gateway marker,
pass `{ gatewayResultMarkers: ['custompaymentresult'] }` as the parser's third
argument.
...
| `malformed` | Intentionally invalid `show_order` payload; this should display `Invalid payment step result`. |
...
`RefId`; the pending poll must remain JSON. No real payment, cart, or
