...
The payment step is the last interactive step: its CTA (**انجام پرداخت** / "Pay now") finalizes the order directly — there is no separate review step. Payment methods show the backend default first, online methods next, and non-default pay-on-delivery/card-to-card choices last. The order-details action on a successful result opens in a new tab.
...
builds, bundled TypeScript declarations, and server-safe entry points:
...
| `@sazito/checkout/next/server` | Next.js-compatible GET/POST gateway callback handlers. |
| `@sazito/checkout/server` | Framework-neutral Web `Request`/`Response` callback handlers. |
...
```bash
NEXT_PUBLIC_SAZITO_DOMAIN=shop.example.com
SAZITO_STORE_DOMAIN=shop.example.com
```

### 2. Add the SDK provider
...
export function Providers({ children }: { children: React.ReactNode }) {
  return <SazitoProvider client={client}>{children}</SazitoProvider>;
}
```

Import the styles and provider once:
...
Replace `<cart-identifier>` with the identifier created through `client.cart`.

### 4. Add the payment callback route
...
That is all the host needs for gateway returns. Keep both `GET` and `POST`:
different gateways use different callback methods. The SDK validates the
callback, verifies the payment on the server, redirects back to `/checkout`,
and shows the final result. Use a Route Handler here, not a Server Action.

### 5. Test the flow

- Open `/checkout` and complete a test payment.
- Confirm the gateway return reaches the catch-all route.
- Confirm the browser returns to `/checkout` and displays the payment result.
...
| `paymentReturn` | `CheckoutPaymentReturn` | Optional callback data for custom integrations. The server handler setup does not require this prop. |
| `paymentReturnParams` | `Record<string, string>` | Legacy query-only callback parameters. The server handler setup does not require this prop. |
| `className` | `string` | Extra class on the root element. |
| `getOrderDetailsUrl` | `(order: CheckoutOrder) => string \| null` | Build the result screen's order-details link using your theme's routes and host. Return `null` to hide it. |
...
already-created invoice. Payment credentials are restored automatically after
the server callback handler redirects to checkout.
...
| `debug` | `boolean` | `false` | Log detailed, correlated payment-verification parsing and checkout state transitions. Sensitive values are redacted. |
...
Gateway callbacks use a nested URL such as:

```text
/checkout/paymentinplaceresult/payment/304/identifier/6f8a717eb211f0839e522a5358932968
```

The catch-all Route Handler from the setup section receives this URL. It parses
the GET or POST fields and verifies the payment. When the API returns
`show_order`, it carries that confirmed result to `checkoutPath` using a small
HTML response and tab-scoped storage. Checkout displays the original success
without calling `process_payment_step` again. If storage is unavailable, the
HTML response itself displays order confirmation. Other responses use a `303`
redirect and a status read; pending results start polling automatically.

Optional server settings:

| Field | Default | Description |
|---|---:|---|
| `checkoutPath` | `/checkout` | Checkout result page. Locale-prefixed paths such as `/fa/checkout` are supported. |
| `maxCallbackBodyBytes` | `65536` | Maximum accepted callback body size. |
| `gatewayResultMarkers` | Built-in Sazito list | Override the accepted callback route names for a custom gateway. |
| `debug` | `false` | Log server-side callback errors without exposing them to the browser. |

The response uses `Cache-Control: no-store` and `Referrer-Policy: no-referrer`.
Malformed callbacks are rejected before the payment API is called. After the
return is recognized, checkout removes the temporary payment parameters from
the browser URL.

### Test the return flow with mock data

The documentation checkout includes local payment-result scenarios, so you can
exercise verification without creating an order or contacting a payment gateway.
Start the docs app and open the demo:
...
```text
http://localhost:3000/checkout-demo?mockPayment=success-minimal
```

You can also change **Payment result** in the demo toolbar:

| Scenario | Expected result |
|---|---|
| `success-minimal` | Success with only the required order identifiers. This protects against the former `Invalid payment step result` failure when totals are absent. |
| `success-full` | Sanitized real v2 payment-in-place `show_order` response with product, shipping, totals, receipt, and nullable backend fields. |
| `pending-success` | Pending callback followed by a successful JSON status poll. |
| `failure` | Gateway-declared payment failure. |
| `malformed` | Incomplete `show_order` payload; this should remain a compact success instead of displaying a false failure. |
| `live` | Normal checkout using the configured domain and cart identifier. |

Mock mode intercepts only the payment verification and post-success pinch calls.
It also asserts that the initial callback is sent as
`application/x-www-form-urlencoded` with `payment_identifier` and
`payload[RefId]`; the pending poll must remain JSON. No real payment, cart, or
gateway redirect is performed.
...
## Customizing the order detail link

Set `getOrderDetailsUrl` on `SazitoCheckoutPage` (or `SazitoCheckout`) to use your
storefront's order detail route. The callback receives the completed order,
including `id` and `orderIdentifier`. Include both in the URL so the destination
can call `client.orders.get(id, orderIdentifier)`.
...
export function ThemeCheckout() {
  return (
    <SazitoCheckoutPage
      config={{ locale: 'fa', continueShoppingUrl: '/fa/products' }}
...
The returned URL is used as supplied: return a relative path for the current
host, or an absolute URL such as `https://shop.example.com/fa/orders/...` for a
different storefront host. Encode dynamic path and query values. Define the
callback in a Client Component in Next.js, inside the existing `SazitoProvider`.

Without this prop, checkout keeps `/orderinfo/{id}/{identifier}`, using the
origin of `continueShoppingUrl` when it is absolute. Returning `null` hides the
order-details action without falling back to that default. The link continues
to open in a new tab. No callback runs when the payment result has no order.
If you supply `renderResult`, your custom renderer owns its links.
...
`onRetry` returns the checkout to the payment step. `order` is populated when
the payment response includes valid order details; a terminal `show_order`
success may omit it while the order is still being finalized server-side.
...
| `order` | `CheckoutOrder \| undefined` | Convenience reference to `result.order`. Available for valid `show_order` and `pending` responses. |
...
The `children` label changes per step: **ثبت سفارش**, **ثبت اطلاعات ارسال**, **ادامه به پرداخت**, **انجام پرداخت** (Persian) or the equivalent English strings.
