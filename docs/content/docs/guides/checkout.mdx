...
`paymentReturnParams`. The checkout auto-resolves the outcome and shows the
result screen.
...
export default async function CheckoutPage({
  ...
  const paymentReturnParams = getPaymentReturnParams(callback, query);
...
The payment ID and identifier from the path are mapped to the `id` and
`paymentIdentifier` callback fields expected by the SDK. Any gateway query
parameters are forwarded as well. Keep the checkout under the same
`SazitoProvider` and SDK client used to create the payment so its persisted
invoice and payment credentials remain available during verification.
